# here downloading data

names<-c("small","medium","large")
if (0==1){
	cols<-c(486,485,484)
	ci <- data.frame(names,cols)

	library(rjson)
	today <- Sys.Date()
	format(today, format="%y-%m-%d")
	from=today-5
	format(from, format="%y-%m-%d")
	urls<-c(paste('http://dashb-atlas-ssb.cern.ch/dashboard/request.py/getplotdata?batch=1&time=custom&dateFrom=',as.character(from),'&dateTo=',as.character(today),'&columnid=',cols,sep=""))
	ci<-cbind(ci,urls)
	
	rawdata<-list()
	for (u in 1:length(names)){
		print(urls[u])
		document <- fromJSON(file=urls[u],method='C')
		cvs=document["csvdata"]
		rawdata[[names[u]]]<-I(cvs[[1]])
	}
	ci<-cbind(ci,I(rawdata))
	
	save(ci,file="localCopyFTS.data")
	print("loading done")
}

if (0==1){
	load("localCopyFTS.data")

	
	cleaned<-list()
	for (u in 1:length(names)){
		#transforms it into matrix 
		mMat <- matrix(unlist(ci$rawdata[u]), ncol=9, byrow=TRUE)
		
		#removes columns 
		mMat <- mMat[,-c(2,3,6,7,9)]
		
		#transforms matrix into data.frame
		mDF <- as.data.frame(mMat)
		colnames(mDF)[2] <- 'link'
		colnames(mDF)[3] <- 'rate'
		colnames(mDF)[4] <- 'date'
		mDF[,3]<-as.numeric(mMat[,3])
		mDF[,4] <- as.POSIXct(mDF[,4],format="%Y-%m-%dT%H:%M:%S")
		
		#orders it first over link and than over date
		mDFsorted <- mDF[order(mDF$link,mDF$date),]
		
		#prints first 10 rows
		head(mDFsorted,10)
		
		#removing rows with 0 rate
		cleaned[[u]]<-I(data.frame(mDFsorted[mDFsorted$rate>0,]))
	}
	
	save(cleaned,file="cleanFTS.data")
}

	library(psych)
	load("cleanFTS.data")
	j<-1
	cc       <- strsplit(as.vector(cleaned[[j]]$link),"_to_")
    source         <- unlist(cc)[2*(1:length(cleaned[[j]]$link))-1]
    destination    <- unlist(cc)[2*(1:length(cleaned[[j]]$link))  ]
    
	cleaned[[j]]$source<-as.list(source)
	cleaned[[j]]$destination <-as.list(destination)
	
	uSources<-sort(unlist(unique(cleaned[[j]]$source)))
	uDestinations<-sort(unlist(unique(cleaned[[j]]$destination)))
	
	print ("all measurements:")
	print(describe(cleaned[[j]]$rate))
	
	nme<-aggregate(rate~link, data=cleaned[[j]], FUN="length")	
	print(paste("average measurements per link:",mean(nme$rate)))	
	
	me <- aggregate(rate~link, data=cleaned[[j]], FUN="mean")
	print ("per link:")
	print (describe(me$rate))
	print (paste("total bandwidth:",sum(me$rate)))
	
	postscript(file = paste("FTS",names[[j]],"files.ps"),paper="a4")
	 par(mfrow = c(2, 1))
	 plot (x=c(1:length(me$link)),y=me$rate,main=paste("FTS",names[[j]],"files"), xlab="link number", ylab="MB/s",type="h")
	 hist(me$rate,main=paste("FTS",names[[j]],"files"),ylab="count", xlab="rate MB/s")
    dev.off()
	
	ma<-aggregate(rate~unlist(source) + unlist(destination), data=cleaned[[j]], FUN="mean")	
	m <- matrix(NA, nrow = length(uSources), ncol = length(uDestinations), byrow = FALSE, dimnames = list(uSources, uDestinations))
	for(i in 1:nrow(ma)){	 m[rownames(m)==ma[i,1],colnames(m)==ma[i,2]] <- ma[i,3] }
	
	print(m[1:4,1:4])
	
	par(mfrow = c(1, 1))
	par(plt=c(0.2,0.98,0.2,0.95) )
	
	image(c(1:length(uSources)), c(1:length(uDestinations)), m, # xlim=c(0,86),ylim=c(0,86),
		col =topo.colors(12), #c("blue","green","red")   #key.axes = axis(4, seq(0, 200, by = 10)),
		axes=FALSE,xlab="Sources",ylab="Destinations",main=paste("FTS",names[[j]],"files"),)
	
	axis(1,at=c(1:length(uSources)),labels=uSources, cex.axis=0.4,las=2)
	axis(2,at=c(1:length(uDestinations)),labels= uDestinations, cex.axis=0.4,las=2)
	
	par(mfrow = c(4, 2))
	par(plt=c(0.2,0.95,0.2,0.8) )
	#splits per link
	aLink<-split(cleaned[[j]], cleaned[[j]]$link)
	for (r in 2:9)
		plot(x=aLink[[r]]$date, y=aLink[[r]]$rate,  main=paste(paste(aLink[[r]]$source[[1]],"to",aLink[[r]]$destination[[1]])), xlab='date', ylab='rate [MB/s]', type="l", col="blue")
	
