...
fn='/atlas/dq2/user/ilijav/HCtest/user.ilijav.HCtest.1/group.test.hc.NTUP_SMWZ.root'
...
with open('checkAll.sh', 'w') as f: # first check that site itself gives it's own file
    for s in sites:
        logfile='checkOf_'+s.name+'.log'
        f.write('echo '+s.host+'  >& '+logfile+' \n')
        # f.write('xrdfs '+s.host+' query stats null >>& '+logfile+' \n')
        f.write('xrdfs '+s.host+ ' stat '+ fn + ' >>& '+logfile+'  \n')
    f.close()
